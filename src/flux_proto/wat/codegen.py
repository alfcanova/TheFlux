...
class _WatCodegen:
    ...
    def _infer_type(self, node: ASTNode) -> str:
        ...
        if isinstance(node, FieldAccess):
            ...
            if node.field == "val":
                ...
                elif node.obj is not None:
                    ot = self._infer_type(node.obj)
                    if ot not in ("data", "result", ""):
                        return ot
                return "data"
    ...
    def _emit_helpers(self, lines: list[str], I: str) -> None:
        ...
        lines.append(f"{I})")
        lines.append("")
        lines.append(f"{I}(func $flux_extract_struct_field (param $s i64) (param $f i64) (result i64)")
        lines.append(f"{I}  (local $s_ptr i32)")
        lines.append(f"{I}  (local $s_len i32)")
        lines.append(f"{I}  (local $f_ptr i32)")
        lines.append(f"{I}  (local $f_len i32)")
        lines.append(f"{I}  (local $limit i32)")
        lines.append(f"{I}  (local $i i32)")
        lines.append(f"{I}  (local $k i32)")
        lines.append(f"{I}  (local $match i32)")
        ...
        lines.append(f"{I}  (local.set $f_ptr (i32.wrap_i64 (i64.shr_u (local.get $f) (i64.const 32))))")
        lines.append(f"{I}  (local.set $f_len (i32.wrap_i64 (i64.and (local.get $f) (i64.const 0xFFFFFFFF))))")
        lines.append(f"{I}  (if (i32.lt_u (local.get $s_len) (local.get $f_len)) (then (return (i64.const 0))))")
        lines.append(f"{I}  (local.set $limit (i32.add (i32.sub (local.get $s_len) (local.get $f_len)) (i32.const 1)))")
        lines.append(f"{I}  (local.set $i (i32.const 0))")
        lines.append(f"{I}  (block $b_found")
        lines.append(f"{I}    (loop $l_search")
        lines.append(f"{I}      (if (i32.ge_u (local.get $i) (local.get $limit)) (then (return (i64.const 0))))")
        lines.append(f"{I}      (local.set $match (i32.const 1))")
        lines.append(f"{I}      (local.set $k (i32.const 0))")
        lines.append(f"{I}      (block $b_chk")
        lines.append(f"{I}        (loop $l_chk")
        lines.append(f"{I}          (if (i32.ge_u (local.get $k) (local.get $f_len)) (then (br $b_chk)))")
        lines.append(f"{I}          (if (i32.ne")
        lines.append(f"{I}                (i32.load8_u (i32.add (i32.add (local.get $s_ptr) (local.get $i)) (local.get $k)))")
        lines.append(f"{I}                (i32.load8_u (i32.add (local.get $f_ptr) (local.get $k))))")
        lines.append(f"{I}            (then (local.set $match (i32.const 0)) (br $b_chk))")
        lines.append(f"{I}          )")
        lines.append(f"{I}          (local.set $k (i32.add (local.get $k) (i32.const 1)))")
        lines.append(f"{I}          (br $l_chk)")
        lines.append(f"{I}        )")
        lines.append(f"{I}      )")
        lines.append(f"{I}      (if (local.get $match) (then (br $b_found)))")
        lines.append(f"{I}      (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        lines.append(f"{I}      (br $l_search)")
        lines.append(f"{I}    )")
        lines.append(f"{I}  )")
        lines.append(f"{I}  (local.set $val_ptr (i32.add (i32.add (local.get $s_ptr) (local.get $i)) (local.get $f_len)))")
        lines.append(f"{I}  (local.set $j (local.get $val_ptr))")
        lines.append(f"{I}  (block $b_end")
        ...
        lines.append(f"{I}        )")
        lines.append(f"{I}      )")
        lines.append(f"{I}      (local.set $j (i32.add (local.get $j) (i32.const 1)))")
        lines.append(f"{I}      (br $l_end)")
        lines.append(f"{I}    )")
        lines.append(f"{I}  )")
        lines.append(f"{I}  (local.set $val_len (i32.sub (local.get $j) (local.get $val_ptr)))")
        lines.append(f"{I}  (return (i64.or (i64.shl (i64.extend_i32_u (local.get $val_ptr)) (i64.const 32)) (i64.extend_i32_u (local.get $val_len))))")
        lines.append(f"{I})")
        lines.append("")
    ...
    def _emit_net_helpers(self, lines: list[str], I: str) -> None:
        L = lines.append
        fat_ok = self._fat_const("OK")
        fat_created = self._fat_const("Created")
        fat_no_content = self._fat_const("No Content")
        fat_moved = self._fat_const("Moved Permanently")
        fat_found = self._fat_const("Found")
        fat_not_modified = self._fat_const("Not Modified")
        fat_bad_request = self._fat_const("Bad Request")
        fat_unauthorized = self._fat_const("Unauthorized")
        fat_forbidden = self._fat_const("Forbidden")
        fat_not_found = self._fat_const("Not Found")
        fat_method_not_allowed = self._fat_const("Method Not Allowed")
        fat_too_many_requests = self._fat_const("Too Many Requests")
        fat_internal_err = self._fat_const("Internal Server Error")
        fat_bad_gateway = self._fat_const("Bad Gateway")
        fat_service_unavailable = self._fat_const("Service Unavailable")
        fat_gateway_timeout = self._fat_const("Gateway Timeout")
        fat_unknown = self._fat_const("Unknown Status")

        # ---- $net_http_status_text ----
        L(f"{I}(func $net_http_status_text (param $c i64) (result i64)")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 200)) (then (return (i64.const {fat_ok}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 201)) (then (return (i64.const {fat_created}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 204)) (then (return (i64.const {fat_no_content}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 301)) (then (return (i64.const {fat_moved}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 302)) (then (return (i64.const {fat_found}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 304)) (then (return (i64.const {fat_not_modified}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 400)) (then (return (i64.const {fat_bad_request}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 401)) (then (return (i64.const {fat_unauthorized}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 403)) (then (return (i64.const {fat_forbidden}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 404)) (then (return (i64.const {fat_not_found}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 405)) (then (return (i64.const {fat_method_not_allowed}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 429)) (then (return (i64.const {fat_too_many_requests}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 500)) (then (return (i64.const {fat_internal_err}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 502)) (then (return (i64.const {fat_bad_gateway}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 503)) (then (return (i64.const {fat_service_unavailable}))))")
        L(f"{I}  (if (i64.eq (local.get $c) (i64.const 504)) (then (return (i64.const {fat_gateway_timeout}))))")
        L(f"{I}  (return (i64.const {fat_unknown}))")
        L(f"{I})")
        L("")

        # ---- $find_scheme_sep : returns index of '://' or -1 ----
        L(f"{I}(func $find_scheme_sep (param $s i64) (result i32)")
        L(f"{I}  (local $ptr i32) (local $len i32) (local $i i32)")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (if (i32.lt_s (local.get $len) (i32.const 3)) (then (return (i32.const -1))))")
        L(f"{I}  (local.set $i (i32.const 0))")
        L(f"{I}  (block $done (loop $lp")
        L(f"{I}    (if (i32.gt_u (local.get $i) (i32.sub (local.get $len) (i32.const 3))) (then (br $done)))")
        L(f"{I}    (if (i32.and (i32.eq (i32.load8_u (i32.add (local.get $ptr) (local.get $i))) (i32.const 58))")
        L(f"{I}        (i32.and (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.add (local.get $i) (i32.const 1)))) (i32.const 47))")
        L(f"{I}                 (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.add (local.get $i) (i32.const 2)))) (i32.const 47))))")
        L(f"{I}      (then (return (local.get $i)))")
        L(f"{I}    )")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $lp)")
        L(f"{I}  ))")
        ...
        # ---- $net_url_get_scheme ----
        L(f"{I}(func $net_url_get_scheme (param $s i64) (result i64)")
        L(f"{I}  (local $pos i32)")
        L(f"{I}  (local.set $pos (call $find_scheme_sep (local.get $s)))")
        L(f"{I}  (if (i32.ge_s (local.get $pos) (i32.const 0))")
        L(f"{I}    (then (return (i64.or (i64.and (local.get $s) (i64.const -4294967296)) (i64.extend_i32_u (local.get $pos))))))")
        L(f"{I}  (return (i64.const 0))")
        L(f"{I})")
        L("")

        # ---- $net_url_get_host ----
        L(f"{I}(func $net_url_get_host (param $s i64) (result i64)")
        L(f"{I}  (local $pos i32) (local $ptr i32) (local $len i32) (local $start i32) (local $i i32) (local $c i32)")
        L(f"{I}  (local.set $pos (call $find_scheme_sep (local.get $s)))")
        L(f"{I}  (if (i32.lt_s (local.get $pos) (i32.const 0)) (then (return (i64.const 0))))")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (local.set $start (i32.add (local.get $pos) (i32.const 3)))")
        L(f"{I}  (local.set $i (local.get $start))")
        L(f"{I}  (block $done (loop $lp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (br $done)))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.or (i32.eq (local.get $c) (i32.const 47))")
        L(f"{I}        (i32.or (i32.eq (local.get $c) (i32.const 58))")
        L(f"{I}        (i32.or (i32.eq (local.get $c) (i32.const 63))")
        L(f"{I}                (i32.eq (local.get $c) (i32.const 35)))))")
        L(f"{I}      (then (br $done))")
        L(f"{I}    )")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $lp)")
        L(f"{I}  ))")
        L(f"{I}  (return (i64.or (i64.shl (i64.extend_i32_u (i32.add (local.get $ptr) (local.get $start))) (i64.const 32))")
        L(f"{I}                  (i64.extend_i32_u (i32.sub (local.get $i) (local.get $start)))))")
        L(f"{I})")
        L("")

        # ---- $net_url_get_port ----
        L(f"{I}(func $net_url_get_port (param $s i64) (result i64)")
        L(f"{I}  (local $pos i32) (local $ptr i32) (local $len i32) (local $i i32) (local $c i32) (local $val i64) (local $found_colon i32)")
        L(f"{I}  (local.set $pos (call $find_scheme_sep (local.get $s)))")
        L(f"{I}  (if (i32.lt_s (local.get $pos) (i32.const 0)) (then (return (i64.const 0))))")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (local.set $i (i32.add (local.get $pos) (i32.const 3)))")
        L(f"{I}  (local.set $found_colon (i32.const 0))")
        L(f"{I}  (block $done (loop $lp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (br $done)))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.eq (local.get $c) (i32.const 58)) (then (local.set $found_colon (i32.const 1)) (local.set $i (i32.add (local.get $i) (i32.const 1))) (br $done)))")
        L(f"{I}    (if (i32.or (i32.eq (local.get $c) (i32.const 47)) (i32.or (i32.eq (local.get $c) (i32.const 63)) (i32.eq (local.get $c) (i32.const 35)))) (then (br $done)))")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $lp)")
        L(f"{I}  ))")
        L(f"{I}  (if (i32.eqz (local.get $found_colon)) (then (return (i64.const 0))))")
        L(f"{I}  (local.set $val (i64.const 0))")
        L(f"{I}  (block $pdone (loop $plp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (br $pdone)))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.or (i32.lt_u (local.get $c) (i32.const 48)) (i32.gt_u (local.get $c) (i32.const 57))) (then (br $pdone)))")
        L(f"{I}    (local.set $val (i64.add (i64.mul (local.get $val) (i64.const 10)) (i64.extend_i32_u (i32.sub (local.get $c) (i32.const 48)))))")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $plp)")
        L(f"{I}  ))")
        L(f"{I}  (return (local.get $val))")
        L(f"{I})")
        L("")

        # ---- $net_url_get_path ----
        L(f"{I}(func $net_url_get_path (param $s i64) (result i64)")
        L(f"{I}  (local $pos i32) (local $ptr i32) (local $len i32) (local $i i32) (local $c i32) (local $start i32)")
        L(f"{I}  (local.set $pos (call $find_scheme_sep (local.get $s)))")
        L(f"{I}  (local.set $start (select (i32.add (local.get $pos) (i32.const 3)) (i32.const 0) (i32.ge_s (local.get $pos) (i32.const 0))))")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (local.set $i (local.get $start))")
        L(f"{I}  (block $find_slash (loop $flp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (return (i64.const 0))))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.eq (local.get $c) (i32.const 47)) (then (br $find_slash)))")
        L(f"{I}    (if (i32.or (i32.eq (local.get $c) (i32.const 63)) (i32.eq (local.get $c) (i32.const 35))) (then (return (i64.const 0))))")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $flp)")
        L(f"{I}  ))")
        L(f"{I}  (local.set $start (local.get $i))")
        L(f"{I}  (block $path_end (loop $pelp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (br $path_end)))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.or (i32.eq (local.get $c) (i32.const 63)) (i32.eq (local.get $c) (i32.const 35))) (then (br $path_end)))")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $pelp)")
        L(f"{I}  ))")
        L(f"{I}  (return (i64.or (i64.shl (i64.extend_i32_u (i32.add (local.get $ptr) (local.get $start))) (i64.const 32))")
        ...
        # ---- $net_url_get_query ----
        L(f"{I}(func $net_url_get_query (param $s i64) (result i64)")
        L(f"{I}  (local $ptr i32) (local $len i32) (local $i i32) (local $c i32) (local $start i32)")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (local.set $i (i32.const 0))")
        L(f"{I}  (block $find_q (loop $qlp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (return (i64.const 0))))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.eq (local.get $c) (i32.const 63)) (then (br $find_q)))")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $qlp)")
        L(f"{I}  ))")
        L(f"{I}  (local.set $start (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}  (local.set $i (local.get $start))")
        L(f"{I}  (block $q_end (loop $qelp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (br $q_end)))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.eq (local.get $c) (i32.const 35)) (then (br $q_end)))")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $qelp)")
        L(f"{I}  ))")
        L(f"{I}  (return (i64.or (i64.shl (i64.extend_i32_u (i32.add (local.get $ptr) (local.get $start))) (i64.const 32))")
        ...
        # ---- $net_url_get_fragment ----
        L(f"{I}(func $net_url_get_fragment (param $s i64) (result i64)")
        L(f"{I}  (local $ptr i32) (local $len i32) (local $i i32) (local $c i32) (local $start i32)")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (local.set $i (i32.const 0))")
        L(f"{I}  (block $find_hash (loop $hlp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (return (i64.const 0))))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.eq (local.get $c) (i32.const 35)) (then (br $find_hash)))")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $hlp)")
        L(f"{I}  ))")
        L(f"{I}  (local.set $start (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}  (return (i64.or (i64.shl (i64.extend_i32_u (i32.add (local.get $ptr) (local.get $start))) (i64.const 32))")
        L(f"{I}                  (i64.extend_i32_u (i32.sub (local.get $len) (local.get $start)))))")
        L(f"{I})")
        L("")

        # ---- $net_url_is_valid ----
        L(f"{I}(func $net_url_is_valid (param $s i64) (result i32)")
        L(f"{I}  (local $pos i32)")
        L(f"{I}  (local.set $pos (call $find_scheme_sep (local.get $s)))")
        L(f"{I}  (return (i32.gt_s (local.get $pos) (i32.const 0)))")
        L(f"{I})")
        L("")

        # ---- $strconcat ----
        L(f"{I}(func $strconcat (param $a i64) (param $b i64) (result i64)")
        L(f"{I}  (local $buf i32) (local $la i32) (local $lb i32)")
        L(f"{I}  (local.set $la (i32.wrap_i64 (local.get $a)))")
        L(f"{I}  (local.set $lb (i32.wrap_i64 (local.get $b)))")
        L(f"{I}  (local.set $buf (call $strbuf_new (i32.add (i32.add (local.get $la) (local.get $lb)) (i32.const 16))))")
        L(f"{I}  (call $strappend (local.get $buf) (local.get $a))")
        L(f"{I}  (call $strappend (local.get $buf) (local.get $b))")
        L(f"{I}  (call $strbuf_done (local.get $buf))")
        L(f"{I}  (return (i64.or (i64.shl (i64.extend_i32_u (i32.add (local.get $buf) (i32.const 4))) (i64.const 32)) (i64.extend_i32_u (i32.load (local.get $buf)))))")
        L(f"{I})")
        L("")
        ...
        # ---- $net_url_join ----
        L(f"{I}(func $net_url_join (param $b i64) (param $r i64) (result i64)")
        L(f"{I}  (local $pb i32) (local $lb i32) (local $pr i32) (local $lr i32) (local $b_slash i32) (local $r_slash i32)")
        L(f"{I}  (if (i64.eqz (local.get $b)) (then (return (local.get $r))))")
        L(f"{I}  (if (i64.eqz (local.get $r)) (then (return (local.get $b))))")
        L(f"{I}  (local.set $pb (i32.wrap_i64 (i64.shr_u (local.get $b) (i64.const 32))))")
        L(f"{I}  (local.set $lb (i32.wrap_i64 (local.get $b)))")
        L(f"{I}  (local.set $pr (i32.wrap_i64 (i64.shr_u (local.get $r) (i64.const 32))))")
        L(f"{I}  (local.set $lr (i32.wrap_i64 (local.get $r)))")
        L(f"{I}  (local.set $b_slash (i32.eq (i32.load8_u (i32.add (local.get $pb) (i32.sub (local.get $lb) (i32.const 1)))) (i32.const 47)))")
        L(f"{I}  (local.set $r_slash (i32.eq (i32.load8_u (local.get $pr)) (i32.const 47)))")
        L(f"{I}  (if (i32.and (local.get $b_slash) (local.get $r_slash))")
        L(f"{I}    (then")
        L(f"{I}      (local.set $pr (i32.add (local.get $pr) (i32.const 1)))")
        L(f"{I}      (local.set $lr (i32.sub (local.get $lr) (i32.const 1)))")
        L(f"{I}      (local.set $r (i64.or (i64.shl (i64.extend_i32_u (local.get $pr)) (i64.const 32)) (i64.extend_i32_u (local.get $lr))))")
        L(f"{I}      (return (call $strconcat (local.get $b) (local.get $r)))")
        L(f"{I}    )")
        L(f"{I}  )")
        L(f"{I}  (if (i32.or (local.get $b_slash) (local.get $r_slash))")
        L(f"{I}    (then (return (call $strconcat (local.get $b) (local.get $r))))")
        L(f"{I}  )")
        L(f"{I}  (return (call $path_join (local.get $b) (local.get $r)))")
        L(f"{I})")
        L("")

        # ---- $net_url_encode ----
        L(f"{I}(func $net_url_encode (param $s i64) (result i64)")
        L(f"{I}  (local $ptr i32) (local $len i32) (local $i i32) (local $c i32) (local $buf i32) (local $out_ptr i32) (local $out_len i32) (local $h1 i32) (local $h2 i32)")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (local.set $buf (call $strbuf_new (i32.add (i32.mul (local.get $len) (i32.const 3)) (i32.const 16))))")
        L(f"{I}  (local.set $out_ptr (i32.add (local.get $buf) (i32.const 4)))")
        L(f"{I}  (local.set $out_len (i32.const 0))")
        L(f"{I}  (local.set $i (i32.const 0))")
        L(f"{I}  (block $done (loop $lp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (br $done)))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.or (i32.and (i32.ge_u (local.get $c) (i32.const 97)) (i32.le_u (local.get $c) (i32.const 122)))")
        L(f"{I}        (i32.or (i32.and (i32.ge_u (local.get $c) (i32.const 65)) (i32.le_u (local.get $c) (i32.const 90)))")
        L(f"{I}        (i32.or (i32.and (i32.ge_u (local.get $c) (i32.const 48)) (i32.le_u (local.get $c) (i32.const 57)))")
        L(f"{I}        (i32.or (i32.eq (local.get $c) (i32.const 45))")
        L(f"{I}        (i32.or (i32.eq (local.get $c) (i32.const 95))")
        L(f"{I}        (i32.or (i32.eq (local.get $c) (i32.const 46))")
        L(f"{I}                (i32.eq (local.get $c) (i32.const 126))))))))")
        L(f"{I}      (then")
        L(f"{I}        (i32.store8 (i32.add (local.get $out_ptr) (local.get $out_len)) (local.get $c))")
        L(f"{I}        (local.set $out_len (i32.add (local.get $out_len) (i32.const 1)))")
        L(f"{I}      )")
        L(f"{I}      (else")
        L(f"{I}        (i32.store8 (i32.add (local.get $out_ptr) (local.get $out_len)) (i32.const 37))")
        L(f"{I}        (local.set $h1 (i32.shr_u (local.get $c) (i32.const 4)))")
        L(f"{I}        (local.set $h2 (i32.and (local.get $c) (i32.const 15)))")
        L(f"{I}        (i32.store8 (i32.add (local.get $out_ptr) (i32.add (local.get $out_len) (i32.const 1)))")
        L(f"{I}                    (select (i32.add (local.get $h1) (i32.const 48)) (i32.add (local.get $h1) (i32.const 55)) (i32.lt_u (local.get $h1) (i32.const 10))))")
        L(f"{I}        (i32.store8 (i32.add (local.get $out_ptr) (i32.add (local.get $out_len) (i32.const 2)))")
        L(f"{I}                    (select (i32.add (local.get $h2) (i32.const 48)) (i32.add (local.get $h2) (i32.const 55)) (i32.lt_u (local.get $h2) (i32.const 10))))")
        L(f"{I}        (local.set $out_len (i32.add (local.get $out_len) (i32.const 3)))")
        L(f"{I}      )")
        L(f"{I}    )")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $lp)")
        L(f"{I}  ))")
        L(f"{I}  (i32.store (local.get $buf) (local.get $out_len))")
        L(f"{I}  (call $strbuf_done (local.get $buf))")
        L(f"{I}  (return (i64.or (i64.shl (i64.extend_i32_u (local.get $out_ptr)) (i64.const 32))")
        L(f"{I}                  (i64.extend_i32_u (local.get $out_len))))")
        L(f"{I})")
        L("")

        # ---- $hex_val & $net_url_decode ----
        L(f"{I}(func $hex_val (param $c i32) (result i32)")
        L(f"{I}  (if (i32.and (i32.ge_u (local.get $c) (i32.const 48)) (i32.le_u (local.get $c) (i32.const 57))) (then (return (i32.sub (local.get $c) (i32.const 48)))))")
        L(f"{I}  (if (i32.and (i32.ge_u (local.get $c) (i32.const 65)) (i32.le_u (local.get $c) (i32.const 70))) (then (return (i32.add (i32.sub (local.get $c) (i32.const 65)) (i32.const 10)))))")
        L(f"{I}  (if (i32.and (i32.ge_u (local.get $c) (i32.const 97)) (i32.le_u (local.get $c) (i32.const 102))) (then (return (i32.add (i32.sub (local.get $c) (i32.const 97)) (i32.const 10)))))")
        L(f"{I}  (return (i32.const -1))")
        L(f"{I})")
        L("")
        L(f"{I}(func $net_url_decode (param $s i64) (result i64)")
        L(f"{I}  (local $ptr i32) (local $len i32) (local $i i32) (local $c i32) (local $buf i32) (local $out_ptr i32) (local $out_len i32) (local $h1 i32) (local $h2 i32)")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (local.set $buf (call $strbuf_new (i32.add (local.get $len) (i32.const 16))))")
        L(f"{I}  (local.set $out_ptr (i32.add (local.get $buf) (i32.const 4)))")
        L(f"{I}  (local.set $out_len (i32.const 0))")
        L(f"{I}  (local.set $i (i32.const 0))")
        L(f"{I}  (block $done (loop $lp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (br $done)))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.and (i32.eq (local.get $c) (i32.const 37)) (i32.le_u (i32.add (local.get $i) (i32.const 2)) (local.get $len)))")
        L(f"{I}      (then")
        L(f"{I}        (local.set $h1 (call $hex_val (i32.load8_u (i32.add (local.get $ptr) (i32.add (local.get $i) (i32.const 1))))))")
        L(f"{I}        (local.set $h2 (call $hex_val (i32.load8_u (i32.add (local.get $ptr) (i32.add (local.get $i) (i32.const 2))))))")
        L(f"{I}        (if (i32.and (i32.ge_s (local.get $h1) (i32.const 0)) (i32.ge_s (local.get $h2) (i32.const 0)))")
        L(f"{I}          (then")
        L(f"{I}            (i32.store8 (i32.add (local.get $out_ptr) (local.get $out_len)) (i32.or (i32.shl (local.get $h1) (i32.const 4)) (local.get $h2)))")
        L(f"{I}            (local.set $out_len (i32.add (local.get $out_len) (i32.const 1)))")
        L(f"{I}            (local.set $i (i32.add (local.get $i) (i32.const 3)))")
        L(f"{I}            (br $lp)")
        L(f"{I}          )")
        L(f"{I}        )")
        L(f"{I}      )")
        ...
        # ---- $net_ip_is_v4 ----
        L(f"{I}(func $net_ip_is_v4 (param $s i64) (result i32)")
        L(f"{I}  (local $ptr i32) (local $len i32) (local $i i32) (local $c i32) (local $val i32) (local $octets i32) (local $digits i32)")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (if (i32.eqz (local.get $len)) (then (return (i32.const 0))))")
        L(f"{I}  (local.set $octets (i32.const 0))")
        L(f"{I}  (local.set $val (i32.const 0))")
        L(f"{I}  (local.set $digits (i32.const 0))")
        L(f"{I}  (local.set $i (i32.const 0))")
        L(f"{I}  (block $done (loop $lp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (br $done)))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.and (i32.ge_u (local.get $c) (i32.const 48)) (i32.le_u (local.get $c) (i32.const 57)))")
        L(f"{I}      (then")
        L(f"{I}        (local.set $val (i32.add (i32.mul (local.get $val) (i32.const 10)) (i32.sub (local.get $c) (i32.const 48))))")
        L(f"{I}        (local.set $digits (i32.add (local.get $digits) (i32.const 1)))")
        L(f"{I}        (if (i32.or (i32.gt_u (local.get $val) (i32.const 255)) (i32.gt_u (local.get $digits) (i32.const 3))) (then (return (i32.const 0))))")
        L(f"{I}      )")
        L(f"{I}      (else")
        L(f"{I}        (if (i32.eq (local.get $c) (i32.const 46))")
        L(f"{I}          (then")
        L(f"{I}            (if (i32.eqz (local.get $digits)) (then (return (i32.const 0))))")
        L(f"{I}            (local.set $octets (i32.add (local.get $octets) (i32.const 1)))")
        L(f"{I}            (local.set $val (i32.const 0))")
        L(f"{I}            (local.set $digits (i32.const 0))")
        L(f"{I}          )")
        L(f"{I}          (else (return (i32.const 0)))")
        L(f"{I}        )")
        L(f"{I}      )")
        L(f"{I}    )")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $lp)")
        L(f"{I}  ))")
        L(f"{I}  (return (i32.and (i32.eq (local.get $octets) (i32.const 3)) (i32.gt_u (local.get $digits) (i32.const 0))))")
        L(f"{I})")
        L("")

        # ---- $net_ip_is_v6 ----
        L(f"{I}(func $net_ip_is_v6 (param $s i64) (result i32)")
        L(f"{I}  (local $ptr i32) (local $len i32) (local $i i32) (local $c i32) (local $colons i32)")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (if (i32.lt_u (local.get $len) (i32.const 2)) (then (return (i32.const 0))))")
        L(f"{I}  (local.set $colons (i32.const 0))")
        L(f"{I}  (local.set $i (i32.const 0))")
        L(f"{I}  (block $done (loop $lp")
        L(f"{I}    (if (i32.ge_u (local.get $i) (local.get $len)) (then (br $done)))")
        L(f"{I}    (local.set $c (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))")
        L(f"{I}    (if (i32.eq (local.get $c) (i32.const 58))")
        L(f"{I}      (then (local.set $colons (i32.add (local.get $colons) (i32.const 1))))")
        L(f"{I}      (else")
        L(f"{I}        (if (i32.eqz (i32.or (i32.and (i32.ge_u (local.get $c) (i32.const 48)) (i32.le_u (local.get $c) (i32.const 57)))")
        L(f"{I}                     (i32.or (i32.and (i32.ge_u (local.get $c) (i32.const 97)) (i32.le_u (local.get $c) (i32.const 102)))")
        L(f"{I}                     (i32.or (i32.and (i32.ge_u (local.get $c) (i32.const 65)) (i32.le_u (local.get $c) (i32.const 70)))")
        L(f"{I}                             (i32.eq (local.get $c) (i32.const 46))))))")
        L(f"{I}          (then (return (i32.const 0)))")
        L(f"{I}        )")
        L(f"{I}      )")
        L(f"{I}    )")
        L(f"{I}    (local.set $i (i32.add (local.get $i) (i32.const 1)))")
        L(f"{I}    (br $lp)")
        L(f"{I}  ))")
        L(f"{I}  (return (i32.ge_u (local.get $colons) (i32.const 2)))")
        L(f"{I})")
        L("")

        # ---- $net_ip_is_valid ----
        L(f"{I}(func $net_ip_is_valid (param $s i64) (result i32)")
        L(f"{I}  (return (i32.or (call $net_ip_is_v4 (local.get $s)) (call $net_ip_is_v6 (local.get $s))))")
        L(f"{I})")
        L("")

        # ---- $net_ip_is_loopback ----
        L(f"{I}(func $net_ip_is_loopback (param $s i64) (result i32)")
        L(f"{I}  (local $ptr i32) (local $len i32)")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (if (i32.ge_u (local.get $len) (i32.const 4))")
        L(f"{I}    (then")
        L(f"{I}      (if (i32.and (i32.eq (i32.load8_u (local.get $ptr)) (i32.const 49))")
        L(f"{I}          (i32.and (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.const 1))) (i32.const 50))")
        L(f"{I}          (i32.and (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.const 2))) (i32.const 55))")
        L(f"{I}                   (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.const 3))) (i32.const 46)))))")
        L(f"{I}        (then (return (i32.const 1)))")
        L(f"{I}      )")
        L(f"{I}    )")
        L(f"{I}  )")
        L(f"{I}  (if (i32.ge_u (local.get $len) (i32.const 3))")
        L(f"{I}    (then")
        L(f"{I}      (if (i32.and (i32.eq (i32.load8_u (local.get $ptr)) (i32.const 58))")
        L(f"{I}          (i32.and (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.const 1))) (i32.const 58))")
        L(f"{I}                   (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.const 2))) (i32.const 49))))")
        L(f"{I}        (then (return (i32.const 1)))")
        L(f"{I}      )")
        L(f"{I}    )")
        L(f"{I}  )")
        L(f"{I}  (return (i32.const 0))")
        L(f"{I})")
        L("")

        # ---- $net_ip_is_private ----
        L(f"{I}(func $net_ip_is_private (param $s i64) (result i32)")
        L(f"{I}  (local $ptr i32) (local $len i32) (local $c0 i32) (local $c1 i32) (local $c2 i32) (local $sec i32)")
        L(f"{I}  (local.set $ptr (i32.wrap_i64 (i64.shr_u (local.get $s) (i64.const 32))))")
        L(f"{I}  (local.set $len (i32.wrap_i64 (local.get $s)))")
        L(f"{I}  (if (i32.lt_u (local.get $len) (i32.const 3)) (then (return (i32.const 0))))")
        L(f"{I}  (local.set $c0 (i32.load8_u (local.get $ptr)))")
        L(f"{I}  (local.set $c1 (i32.load8_u (i32.add (local.get $ptr) (i32.const 1))))")
        L(f"{I}  (local.set $c2 (i32.load8_u (i32.add (local.get $ptr) (i32.const 2))))")
        L(f"{I}  (if (i32.and (i32.eq (local.get $c0) (i32.const 49))")
        L(f"{I}      (i32.and (i32.eq (local.get $c1) (i32.const 48))")
        L(f"{I}               (i32.eq (local.get $c2) (i32.const 46))))")
        L(f"{I}    (then (return (i32.const 1)))")
        L(f"{I}  )")
        L(f"{I}  (if (i32.ge_u (local.get $len) (i32.const 8))")
        L(f"{I}    (then")
        L(f"{I}      (if (i32.and (i32.eq (local.get $c0) (i32.const 49))")
        L(f"{I}          (i32.and (i32.eq (local.get $c1) (i32.const 57))")
        L(f"{I}          (i32.and (i32.eq (local.get $c2) (i32.const 50))")
        L(f"{I}          (i32.and (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.const 3))) (i32.const 46))")
        L(f"{I}          (i32.and (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.const 4))) (i32.const 49))")
        L(f"{I}          (i32.and (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.const 5))) (i32.const 54))")
        L(f"{I}          (i32.and (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.const 6))) (i32.const 56))")
        L(f"{I}                   (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.const 7))) (i32.const 46)))))))))")
        L(f"{I}        (then (return (i32.const 1)))")
        L(f"{I}      )")
        L(f"{I}    )")
        L(f"{I}  )")
        L(f"{I}  (if (i32.ge_u (local.get $len) (i32.const 7))")
        L(f"{I}    (then")
        L(f"{I}      (if (i32.and (i32.eq (local.get $c0) (i32.const 49))")
        L(f"{I}          (i32.and (i32.eq (local.get $c1) (i32.const 55))")
        L(f"{I}          (i32.and (i32.eq (local.get $c2) (i32.const 50))")
        L(f"{I}                   (i32.eq (i32.load8_u (i32.add (local.get $ptr) (i32.const 3))) (i32.const 46)))))")
        L(f"{I}        (then")
        L(f"{I}          (local.set $sec (i32.add (i32.mul (i32.sub (i32.load8_u (i32.add (local.get $ptr) (i32.const 4))) (i32.const 48)) (i32.const 10))")
        L(f"{I}                                   (i32.sub (i32.load8_u (i32.add (local.get $ptr) (i32.const 5))) (i32.const 48))))")
        L(f"{I}          (if (i32.and (i32.ge_s (local.get $sec) (i32.const 16)) (i32.le_s (local.get $sec) (i32.const 31))) (then (return (i32.const 1))))")
        L(f"{I}        )")
        L(f"{I}      )")
        L(f"{I}    )")
        L(f"{I}  )")
    ...
    def _gen_expr(self, node: ASTNode | None, fb: _FuncBuilder, body: list[str], I: str) -> tuple[str, str]:
        ...
        if isinstance(node, Identifier):
            if node.name in self._struct_slots:
                s_fat = self._gen_struct_to_str(node.name, fb, body, I)
                return (s_fat, "i64")
        ...
        if isinstance(node, FieldAccess):
            ...
            if node.field in ("sta", "val", "msg"):
                ...
                if node.field == "sta":
                    return (self._fat_expr("(local.get $__fr_sta)", "(call $strlen (local.get $__fr_sta))"), "i64")
                if node.field == "msg":
                    return (self._fat_expr("(local.get $__fr_msg)", "(call $strlen (local.get $__fr_msg))"), "i64")
                return ("(i32.const 0)", "i32")
        if isinstance(node, BinaryOp):
        ...
        if n:
            ...
            for i, item in enumerate(node.items, start=1):
                ...
                if vt == "f64":
                    ...
                    v = f"(i64.extend_i32_u {v})"
                if self._is_map_access(item):
                    src_obj, src_vt = self._gen_expr(item.obj, fb, body, I)
                    src_op = fb.new_i32()
                    body.append(f"{I}(local.set {src_op} {self._fit_wat(src_obj, src_vt, 'i32')})")
                    src_kf = self._map_key_fat(item.indices[0], fb, body, I)
                    tag_expr = f"(call $map_get_tag (local.get {src_op}) {src_kf})"
                elif isinstance(item, IndexAccess) and (self._is_list_type(self._infer_type(item.obj)) or self._infer_type(item.obj) == "data"):
                    src_obj, src_vt = self._gen_expr(item.obj, fb, body, I)
                    src_op = fb.new_i32()
                    body.append(f"{I}(local.set {src_op} {self._fit_wat(src_obj, src_vt, 'i32')})")
    ...
    def _gen_elem_print(self, node: IndexAccess, fb: _FuncBuilder, body: list[str], I: str) -> None:
        ...
        is_map = self._is_map_type(bt) or (
            bt in ("data", "result", "")
            and (self._is_str_type(self._infer_type(chain[-1])) or (isinstance(chain[-1], Literal) and chain[-1].value_type.lower() in ("string", "str")))
        )
        if is_map:
            if len(chain) != 1:
    ...
    def _gen_index_access(self, node: IndexAccess, fb: _FuncBuilder, body: list[str], I: str) -> tuple[str, str]:
        ...
        ov, ovt = self._gen_expr(node.obj, fb, body, I)
        is_map = self._is_map_type(ot) or (
            ot in ("data", "result", "")
            and (self._is_str_type(self._infer_type(node.indices[0])) or (isinstance(node.indices[0], Literal) and node.indices[0].value_type.lower() in ("string", "str")))
        )
        if is_map:
            op = fb.new_i32()
    ...
    def _gen_index_assign(self, node: IndexAssign, fb: _FuncBuilder, body: list[str], I: str) -> None:
        ...
        if is_map:
            ...
            if self._is_map_access(node.value):
        ...
        if len(idxs) == 1:
            ...
            if self._is_map_access(node.value):
                ...
                body.append(f"{I}(local.set {src_op} {self._fit_wat(src_obj, src_vt, 'i32')})")
                src_kf = self._map_key_fat(node.value.indices[0], fb, body, I)
                tag = f"(call $map_get_tag (local.get {src_op}) {src_kf})"
            elif isinstance(node.value, IndexAccess) and (self._is_list_type(self._infer_type(node.value.obj)) or self._infer_type(node.value.obj) == "data"):
                src_obj, src_vt = self._gen_expr(node.value.obj, fb, body, I)
                src_op = fb.new_i32()
                body.append(f"{I}(local.set {src_op} {self._fit_wat(src_obj, src_vt, 'i32')})")
    ...
    def _alloc_struct_slots_global(self, prefix: str, sname: str) -> dict:
        sdef = self._structs.get(sname)
        if sdef is None:
            raise WatError(f"struct '{sname}' not declared")
        slots: dict = {"fields": {}, "sname": sdef.name, "kind": "global"}
        for f in sdef.fields:
            fname = f.name
            ...
            if raw_t in self._structs:
                ...
                wt = self._wtype(raw_t)
                gname = f"${prefix}_{fname}"
                slots["fields"][fname] = (gname, wt)
                if raw_t in ("bool", "boolean"):
                    ft = "bool"
                elif wt == "f64":
                    ft = "float64"
                elif self._struct_field_is_str(sdef.name, fname):
                    ft = "string"
                else:
                    ft = "int64"
                self._globals[f"{prefix}_{fname}"] = (ft, wt)
        return slots

    def _alloc_struct_slots_local(self, prefix: str, sname: str, fb: _FuncBuilder) -> dict:
        sdef = self._structs.get(sname)
        if sdef is None:
            raise WatError(f"struct '{sname}' not declared")
        slots: dict = {"fields": {}, "sname": sdef.name, "kind": "local"}
        for f in sdef.fields:
            fname = f.name
            raw_t = f.type_ref.name if f.type_ref else "int64"
            if raw_t in self._structs:
    ...
    def _declare_struct_storage_global(self, it: StorageItem, body: list[str] | None) -> None:
        ...
        self._struct_slots[it.name] = {"kind": "global", "slots": slots, "sname": it.type_ref.name}
        if body is not None and it.initializer is not None:
    ...
    def _declare_struct_storage_local(self, it: StorageItem, fb: _FuncBuilder, body: list[str], I: str) -> None:
        ...
        self._struct_slots[it.name] = {"kind": "local", "slots": slots, "sname": it.type_ref.name}
        if it.initializer:
            self._emit_struct_into(it.initializer, slots, fb, body, I, is_global=False)

    def _resolve_struct_slot(self, node: ASTNode) -> tuple[str, str] | dict | None:
        if isinstance(node, Identifier):
            if node.name in self._struct_slots:
                return self._struct_slots[node.name]["slots"]
            return None
        if isinstance(node, FieldAccess):
    ...
    def _emit_struct_into(self, init: ASTNode | None, slots: dict, fb: _FuncBuilder | None, body: list[str], I: str, is_global: bool) -> None:
        ...
        if init is None:
            for name, target in slots["fields"].items():
                if isinstance(target, dict):
                    self._emit_struct_into(None, target, fb, body, I, is_global=is_global)
                else:
                    sid, wt = target
                    zero = "(i64.const 0)" if wt == "i64" else ("(f64.const 0.0)" if wt == "f64" else "(i32.const 0)")
                    body.append(f"{I}({op} {sid} {zero})")
        ...
        for f in init.fields:
            ...
            target = slots["fields"][f.name]
            if isinstance(target, dict):
                self._emit_struct_into(f.value, target, fb, body, I, is_global=is_global)
            else:
                lid, wt = target
                val, _ = self._gen_expr(f.value, fb, body, I)
                body.append(f"{I}({op} {lid} {val})")

    def _gen_struct_slots_to_str(self, info: dict, fb: _FuncBuilder, body: list[str], I: str) -> str:
        ...
        for j, f in enumerate(sdef.fields):
            ...
            target = slots["fields"][f.name]
            if isinstance(target, dict):
                nested_fat = self._gen_struct_slots_to_str(target, fb, body, I)
                body.append(f"{I}(call $strappend (local.get {buf}) {nested_fat})")
            else:
                wt = target[1]
                slot_id = target[0]
                ...
                if ft == "datetime":
                    body.append(f"{I}(local.set {tt} (call $flux_alloc (i32.const 64)))")
                    body.append(f"{I}(local.set {cnt} (call $dt_to_str ({sget} {slot_id}) (local.get {tt})))")
        ...
        res_fat = fb.new_i64()
        body.append(f"{I}(local.set {res_fat} {self._fat_expr(f'(i32.add (local.get {buf}) (i32.const 4))', f'(i32.load (local.get {buf}))')})")
        return f"(local.get {res_fat})"

    def _gen_struct_to_str(self, name: str, fb: _FuncBuilder, body: list[str], I: str) -> str:
        return self._gen_struct_slots_to_str(self._struct_slots[name]["slots"], fb, body, I)
    ...
    def _gen_field_assign(self, node: FieldAssign, fb: _FuncBuilder, body: list[str], I: str) -> None:
        ...
        if isinstance(target, dict):
            if not isinstance(node.value, StructInit):
                raise WatError(f"struct field '{node.field}' requires a StructInit value")
            self._emit_struct_into(node.value, target, fb, body, I, is_global=(target.get("kind") == "global"))
            return
        lid, wt = target
    ...
    def _field_type(self, node: FieldAccess) -> str | None:
        parent = self._resolve_struct_slot(node.obj)
        if isinstance(parent, dict) and "sname" in parent:
            sdef = self._structs.get(parent["sname"])
            if sdef is not None:
                for f in sdef.fields:
        ...
        for i, m in enumerate(edef.members):
            ...
            if not m.fields:
                ...
                for j, f in enumerate(m.fields):
                    ...
                    if wt == "f64":
                        self._append_f64(f"({get} {slot_id})", fb, body, I + "    ", buf, tt, cnt, fat)
    ...
    def _gen_print_arg(self, node: ASTNode, newline: bool, fb: _FuncBuilder, body: list[str], I: str) -> None:
        ...
        for p in parts:
            ...
            if isinstance(p, Literal) and p.value_type.lower() == "char":
                ...
                val, vt = self._gen_expr(p, fb, body, I)
                v64 = self._fit_wat(val, vt, "i64")
                buf = fb.new_i32()
                cnt = fb.new_i32()
                buf_off = self._itoa_buf_off()
                body.append(f"{I}(local.set {buf} (i32.const {buf_off}))")
                body.append(f"{I}(local.set {cnt} (call $i64_to_str {v64} (local.get {buf})))")
                body.append(f"{I}(call $print_str (local.get {buf}) (local.get {cnt}))")
        if newline:
            nl_off = self._alloc_str("\n")
            body.append(f"{I}(call $print_str_const (i32.const {nl_off}) (i32.const 1))")
    ...
    def _gen_call(self, node: CallExpr, fb: _FuncBuilder, body: list[str], I: str) -> tuple[str, str]:
        ...
        if name.startswith("stdOs"):
            ...
            if name == "stdOsSetEnv":
                nv, _ = self._gen_expr(node.args[0], fb, body, I)
                vv, _ = self._gen_expr(node.args[1], fb, body, I)
                body.append(f"{I}(global.set $flux_os_env_name {nv})")
                body.append(f"{I}(global.set $flux_os_env_val {vv})")
                body.append(f"{I}(global.set $flux_os_env_has (i32.const 1))")
                return ("(i32.const 1)", "i32")
            if name == "stdOsUnsetEnv":
                nv, _ = self._gen_expr(node.args[0], fb, body, I)
                body.append(f"{I}(if (call $streq {nv} (global.get $flux_os_env_name)) (then (global.set $flux_os_env_has (i32.const 0))))")
                return ("(i32.const 1)", "i32")
            if name == "stdOsHasEnv":
                nv, _ = self._gen_expr(node.args[0], fb, body, I)
                res = f"(i32.and (global.get $flux_os_env_has) (call $streq {nv} (global.get $flux_os_env_name)))"
                return (res, "i32")
            if name == "stdOsGetEnv":
                nv, _ = self._gen_expr(node.args[0], fb, body, I)
                empty_fat = self._fat_const("")
                cond = f"(i32.and (global.get $flux_os_env_has) (call $streq {nv} (global.get $flux_os_env_name)))"
                res = f"(select (global.get $flux_os_env_val) (i64.const {empty_fat}) {cond})"
                return (res, "i64")
            if name == "stdOsGetEnvOrDefault":
                nv, _ = self._gen_expr(node.args[0], fb, body, I)
                dv, _ = self._gen_expr(node.args[1], fb, body, I)
                cond = f"(i32.and (global.get $flux_os_env_has) (call $streq {nv} (global.get $flux_os_env_name)))"
                res = f"(select (global.get $flux_os_env_val) {dv} {cond})"
                return (res, "i64")
            for a in node.args:
                v, _ = self._gen_expr(a, fb, body, I)
                body.append(f"{I}(drop {v})")
            if name == "stdOsListEnv":
                ...
                body.append(f"{I}(call $list_set_row (local.get {lp}) (i32.const 1) (i32.const 4) (i64.const {fat_v}))")
                return (f"(i64.extend_i32_u (local.get {lp}))", "i64")
            ...
            if name == "stdOsDirSeparator":
                ...
                return (f"(i64.const {fat})", "i64")
            if name == "stdOsGetPid":
                return ("(i64.const 1001)", "i64")
            if name == "stdOsGetParentPid":
                return ("(i64.const 1000)", "i64")
            if name == "stdOsCwd":
                val = osh.os_cwd()
                fat = self._fat_const(val)
                return (f"(i64.const {fat})", "i64")
            ...
            if name == "stdOsCpuCount":
                ...
                return (f"(i64.const {val})", "i64")
            if name == "stdOsUptime":
                return ("(i64.const 3600)", "i64")
            if name == "stdOsMemoryTotal":
                val = osh.os_memory_total()
                return (f"(i64.const {val})", "i64")
            if name == "stdOsMemoryFree":
                return ("(i64.const 1073741824)", "i64")
        if name.startswith("stdNet"):
            if name == "stdNetUrlGetScheme":
                uv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_url_get_scheme {uv})", "i64")
            if name == "stdNetUrlGetHost":
                uv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_url_get_host {uv})", "i64")
            if name == "stdNetUrlGetPort":
                uv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_url_get_port {uv})", "i64")
            if name == "stdNetUrlGetPath":
                uv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_url_get_path {uv})", "i64")
            if name == "stdNetUrlGetQuery":
                uv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_url_get_query {uv})", "i64")
            if name == "stdNetUrlGetFragment":
                uv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_url_get_fragment {uv})", "i64")
            if name == "stdNetUrlEncode":
                uv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_url_encode {uv})", "i64")
            if name == "stdNetUrlDecode":
                uv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_url_decode {uv})", "i64")
            if name == "stdNetUrlIsValid":
                uv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_url_is_valid {uv})", "i32")
            if name == "stdNetUrlJoin":
                bv, _ = self._gen_expr(node.args[0], fb, body, I)
                rv, _ = self._gen_expr(node.args[1], fb, body, I)
                return (f"(call $net_url_join {bv} {rv})", "i64")
            if name == "stdNetIpIsValid":
                iv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_ip_is_valid {iv})", "i32")
            if name == "stdNetIpIsV4":
                iv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_ip_is_v4 {iv})", "i32")
            if name == "stdNetIpIsV6":
                iv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_ip_is_v6 {iv})", "i32")
            if name == "stdNetIpIsLoopback":
                iv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_ip_is_loopback {iv})", "i32")
            if name == "stdNetIpIsPrivate":
                iv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_ip_is_private {iv})", "i32")
            if name == "stdNetResolveHost":
                hv, _ = self._gen_expr(node.args[0], fb, body, I)
                body.append(f"{I}(drop {hv})")
                fat = self._fat_const("127.0.0.1")
                return (f"(i64.const {fat})", "i64")
            if name == "stdNetResolveIp":
                iv, _ = self._gen_expr(node.args[0], fb, body, I)
                body.append(f"{I}(drop {iv})")
                fat = self._fat_const("localhost")
                return (f"(i64.const {fat})", "i64")
            if name == "stdNetHttpStatusText":
                cv, _ = self._gen_expr(node.args[0], fb, body, I)
                return (f"(call $net_http_status_text {cv})", "i64")
            for a in node.args:
                v, _ = self._gen_expr(a, fb, body, I)
                body.append(f"{I}(drop {v})")
        ...
        if name in self._user_funcs:
            ...
            for k, a in enumerate(node.args):
                ...
                pft = info["params"][k][2] if k < len(info["params"]) and len(info["params"][k]) > 2 else ""
                if pw == "i32":
                    if at == "i32":
                        slot = fb.new_i32()
                        body.append(f"{I}(local.set {slot} {av})")
                    elif self._is_char_type(pft) and (self._is_str_type(self._infer_type(a)) or (isinstance(a, IndexAccess) and self._is_str_type(self._infer_type(a.obj)))):
                        slot = fb.new_i32()
                        str_tmp = fb.new_i64()
                        body.append(f"{I}(local.set {str_tmp} {self._fit_wat(av, at, 'i64')})")
                        char_val = f"(i32.load8_u (i32.wrap_i64 (i64.shr_u (local.get {str_tmp}) (i64.const 32))))"
                        body.append(f"{I}(local.set {slot} {char_val})")
                    else:
                        slot = fb.new_i32()
                ...
                arg_val_slots.append(slot)
            for k, (_, _, pft) in enumerate(info["params"]):
                if pft == "data":
                    if k < len(node.args):
                        arg_node = node.args[k]
                        if isinstance(arg_node, Identifier) and arg_node.name in self._param_tag_slots:
                            arg_slots.append(f"(local.get {self._param_tag_slots[arg_node.name]})")
                        elif isinstance(arg_node, IndexAccess):
                            bt = self._infer_type(arg_node.obj)
                            if self._is_map_type(bt) or bt == "map":
                                src_obj, src_vt = self._gen_expr(arg_node.obj, fb, body, I)
                                src_op = fb.new_i32()
                                body.append(f"{I}(local.set {src_op} {self._fit_wat(src_obj, src_vt, 'i32')})")
                                src_kf = self._map_key_fat(arg_node.indices[0], fb, body, I)
                                tag_loc = fb.new_i32()
                                body.append(f"{I}(local.set {tag_loc} (call $map_get_tag (local.get {src_op}) {src_kf}))")
                                arg_slots.append(f"(local.get {tag_loc})")
                            elif self._is_list_type(bt) or bt == "data":
                                src_obj, src_vt = self._gen_expr(arg_node.obj, fb, body, I)
                                src_op = fb.new_i32()
                                body.append(f"{I}(local.set {src_op} {self._fit_wat(src_obj, src_vt, 'i32')})")
                                src_iv, _ = self._gen_expr(arg_node.indices[0], fb, body, I)
                                tag_loc = fb.new_i32()
                                body.append(f"{I}(local.set {tag_loc} (call $list_row_tag (local.get {src_op}) (i32.wrap_i64 {src_iv})))")
                                arg_slots.append(f"(local.get {tag_loc})")
                            else:
                                arg_slots.append("(i32.const 1)")
                        elif self._infer_type(arg_node) != "data":
                            arg_slots.append(f"(i32.const {self._list_tag_of(self._infer_type(arg_node))})")
                        else:
                            v = f"(local.get {arg_val_slots[k]})"
                            tag_loc = fb.new_i32()
                            body.append(f"{I}(local.set {tag_loc} (select (i32.const 5) (select (i32.const 4) (i32.const 1) (i32.gt_u (i32.wrap_i64 (i64.shr_u {v} (i64.const 32))) (i32.const 0))) (i32.and (i64.eqz (i64.shr_u {v} (i64.const 32))) (i32.ge_u (i32.wrap_i64 {v}) (global.get $flux_heap_start)))))")
                            arg_slots.append(f"(local.get {tag_loc})")
                    else:
                        arg_slots.append("(i32.const 1)")
        ...
        for i, arm in enumerate(node.arms):
            ...
            if isinstance(pat, LiteralPattern):
                ...
                for f in pat.fields:
                    ...
                    if isinstance(target, dict):
                        if isinstance(f.value, IdentifierPattern):
                            self._struct_slots[f.value.name] = {"kind": struct_subject["kind"], "slots": target, "sname": target.get("sname", "")}
                            continue
                        raise WatError("nested struct pattern not supported on WAT target")
                    lid, wt = target
