# function-abi-result-frames

## Purpose

Define a ABI de resultado de chamada de função nos backends VM, LLVM, WAT e WASM: cada invocação SHALL materializar um frame de resultado `{sta, val, msg}` próprio, preservado sob aninhamento e recursão, idêntico ao interpreter, com erro explícito no lugar de valores sintéticos e paridade garantida pelo gate de compliance.
